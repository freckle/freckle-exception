...
-- | An hspec hook that catches and pretty-prints 'HUnitFailure', the exception
-- that is thrown when a test assertion fails
--
-- Tests for any code that might throw 'AnnotatedException' (which includes
-- anything that uses @freckle-exception@) should add this hook to their test
-- suite. Without it, if you end up with an @'AnnotatedException'
-- 'HUnitFailure'@, hspec doesn't recognize it as an assertion failure and you
-- get ugly output instead of nice output.
...
annotateHUnitFailure AnnotatedException {exception = HUnitFailure l x, annotations} =
  HUnitFailure l $ annotateFailureReason annotations x
...
-- | Construct a message that consists of an introductory paragraph plus some
-- additional paragraphs based on annotations, separated by blank lines
makeMessage :: String -> [Annotation] -> String
makeMessage m as =
  combineParagraphs $ pretty m :| annotationParagraphs as

-- | Like 'makeMessage' but without necessarily having an introductory paragraph
-- present.
--
-- If there is neither an introductory paragraph nor any annotations, the result
-- is 'Nothing'.
makeMessageMaybe :: Maybe String -> [Annotation] -> Maybe String
makeMessageMaybe mm as =
  fmap combineParagraphs
    $ nonEmpty
    $ fmap pretty (toList mm)
      <> annotationParagraphs as
